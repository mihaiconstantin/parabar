...
The reminder of this article is organized as follows. I start with a section on
...
the `?parallel::makeCluster` function. The role of a backend is to abide to a
...
`?Backend` and the `?Context` classes share an `sapply` method since both
...
[`pbapply`](https://peter.solymos.org/pbapply) is a a versatile package that
...
line `53` splits the task repetitions into a smaller chunks based on how many
...
three tasks repetitions each. In lines `54` and `55` `pbapply` counts how many
splits were created and starts a progress bar with a minimum of zero and a
maximum value set to the total number of splits (i.e., seven in this case). Even
more interesting are lines `58` to `61`. Here, `pbapply` loops over each split
and calls the selected parallel function (e.g., `?parallel::parLapply`) with the
...
The code snippet above already provides provides us with two important insights
into how `pbapply` works. First, by default, `pbapply` does not update the
progress bar after each task repetition. Second, the overhead `pbapply` adds is
likely given by the repeated calls to the parallel function (e.g.,
`?parallel::parLapply`) that involves transferring the tasks to the different
processes that make up the cluster nodes.
...
times, which in turns results in calling the `?parallel::clusterApply` for each
...
overhead is likely negligible when the the number of tasks is low enough and
repeated calls to the `?parallel::clusterApply` are not needed.

In the next section, I provide a provide rough estimates of the overhead
associated with the progress tracking approaches discussed above.
...
$1000$ repetitions of this task in parallel via the the built-in function
...
function. Note that that the progress tracking functionality is still employed
(i.e., logging the execution progress to a file and reading the file to update
the progress bar), but the progress bar is not displayed.
...
than just a package that adds progress bar for parallelized `R` code. Through
...
different ways, such as, for example, sending an email when the parallelized
task finished executing. Therefore, the progress tracking functionality
discussed in this article is just one example of what is possible with
`parabar`.
